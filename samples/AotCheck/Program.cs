...
var result = parser.Evaluate("max(1, 2, 3) + PI * 2");
Console.WriteLine($"arithmetic:  {result}");

using var doc = JsonDocument.Parse("""{"xs":[1,2,3,4,5],"threshold":3}""");
var filtered = parser.Evaluate("sum(filter(xs, x => x > threshold))", doc);
Console.WriteLine($"higher-order:{filtered}");

var expr = parser.Parse("(a + b) * 2");
var simplified = expr.Simplify(JsonDocument.Parse("""{"a":3,"b":4}"""));
Console.WriteLine($"simplify:    {simplified}");

var sub = expr.Substitute("a", "x * y");
