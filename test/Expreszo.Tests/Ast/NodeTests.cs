...
public class NodeTests
{
    ...
    {
        var a = new Binary("+", new NumberLit(1, Node.NoSpan), new NumberLit(2, Node.NoSpan), Node.NoSpan);
        var b = new Binary("+", new NumberLit(1, Node.NoSpan), new NumberLit(2, Node.NoSpan), Node.NoSpan);
    ...
    {
        var a = new Lambda(ImmutableArray.Create("x", "y"), new Ident("x", Node.NoSpan), Node.NoSpan);
        var b = new Lambda(ImmutableArray.Create("x", "y"), new Ident("x", Node.NoSpan), Node.NoSpan);
        var c = new Lambda(ImmutableArray.Create("y", "x"), new Ident("x", Node.NoSpan), Node.NoSpan);
        await Assert.That(a).IsEqualTo(b);
...
public class NodeVisitorTests
{
    ...
    public async Task Visit_dispatches_on_node_type()
    {
        ...
        Node root = new Binary("+", new Ident("x", Node.NoSpan), new NumberLit(1, Node.NoSpan), Node.NoSpan);
        visitor.Visit(root);
    ...
    public async Task Visit_throws_for_unsupported_node_types_by_default()
    {
        ...
        await Assert.That(() => visitor.Visit(node)).Throws<NotImplementedException>();
    }
}
