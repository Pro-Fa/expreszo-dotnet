...
public class ErrorHierarchyTests
{
    ...
    public async Task Every_specific_exception_is_an_ExpressionException()
    {
        ...
        await Assert.That(new ExpressionArgumentException("x")).IsAssignableTo<ExpressionException>();
    ...
    public async Task ThrowingErrorHandler_returns_Throw_for_all_events()
    {
        ...
        await Assert.That(h.OnParseError(new ParseException("x")))
            .IsTypeOf<ErrorDisposition.Rethrow>();
        await Assert.That(h.OnEvaluationError(new EvaluationException("x")))
            .IsTypeOf<ErrorDisposition.Rethrow>();
