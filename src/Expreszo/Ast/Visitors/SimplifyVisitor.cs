...
/// </summary>
internal sealed class SimplifyVisitor(OperatorTable ops, ParserConfig config, JsonDocument? values)
{
    private readonly Scope _scope = Scope.FromJsonDocument(values);
    ...
    public Node Simplify(Node node) =>
        node switch
        {
            ...
            ArrayLit a => new ArrayLit(
                a.Elements.Select<ArrayEntry, ArrayEntry>(e =>
                        e switch
                        {
            ...
            ObjectLit o => new ObjectLit(
                o.Properties.Select<ObjectEntry, ObjectEntry>(p =>
                        p switch
                        {
                            ObjectProperty pr => new ObjectProperty(
                                pr.Key,
                                Simplify(pr.Value),
                                pr.Quoted,
                                pr.Span
    ...
    {
        if (_scope.TryGet(id.Name, out Value? v))
        {
            return LiteralFor(v, id.Span);
        }
        if (config.NumericConstants.TryGetValue(id.Name, out double nv))
        {
    ...
    {
        Node operand = Simplify(u.Operand);

        if (
            operand is not (NumberLit or StringLit or BoolLit or NullLit or UndefinedLit)
            || !ops.UnaryOps.TryGetValue(u.Op, out ExprFunc? fn)
        )
        {
        ...
        try
        {
            Value v = ValueFor(operand);
            ValueTask<Value> task = fn([v], DummyCtx);
    ...
    {
        Node left = Simplify(b.Left);
        Node right = Simplify(b.Right);
        ...
        try
        {
            ValueTask<Value> task = fn([ValueFor(left), ValueFor(right)], DummyCtx);
            if (!task.IsCompletedSuccessfully)
            {
                return new Binary(b.Op, left, right, b.Span);
            }
    ...
    private Node TrySimplifyTernary(Ternary t)
    {
        ...
        if (t.Op == "?" && IsLiteral(a))
        {
            ...
            Node chosen = ValueFor(a).IsTruthy() ? Unwrap(b) : Unwrap(c);
            return chosen;
        }
    ...
    {
        Node obj = Simplify(m.Object);

        if (
            obj is RawLit { Value: Value.Object o }
            && o.Props.TryGetValue(m.Property, out Value? v)
        )
        {
    ...
    private Case SimplifyCase(Case k) =>
        new(
            k.Subject is null ? null : Simplify(k.Subject),
            k.Arms.Select(arm => new CaseArm(Simplify(arm.When), Simplify(arm.Then)))
                .ToImmutableArray(),
    ...
    private static Value ValueFor(Node n) =>
        n switch
        {
    ...
    private static Node LiteralFor(Value v, TextSpan span) =>
        v switch
        {
    ...
    private static readonly EvalContext DummyCtx = new(new Scope(), ThrowingErrorHandler.Instance);
}
