...
/// </summary>
internal sealed class SubstituteVisitor(string variable, Node replacement)
{
    public Node Substitute(Node node) =>
        node switch
        {
            Ident id when id.Name == variable => replacement,
            Ident
            or NumberLit
            or StringLit
            or BoolLit
            ...
            Lambda l when l.Params.Contains(variable) => l, // variable is shadowed by parameter
            Lambda l => new Lambda(l.Params, Substitute(l.Body), l.Span),
            FunctionDef fd when fd.Params.Contains(variable) => fd,
    ...
    private ArrayLit SubstituteArray(ArrayLit a) =>
        new(
    ...
    private ObjectLit SubstituteObject(ObjectLit o) =>
        new(
    ...
    private Case SubstituteCase(Case k) =>
        new(
            k.Subject is null ? null : Substitute(k.Subject),
            k.Arms.Select(arm => new CaseArm(Substitute(arm.When), Substitute(arm.Then)))
                .ToImmutableArray(),
