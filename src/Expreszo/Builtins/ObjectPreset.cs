...
internal static class ObjectPreset
{
    public static void RegisterInto(OperatorTableBuilder b)
    {
        b.AddFunction(
            ...
            OperatorTableBuilder.Sync(args =>
            {
                ...
                foreach (Value a in args)
                {
                    ...
                    {
                        foreach (KeyValuePair<string, Value> kv in o.Props)
                        {
        ...
        b.AddFunction(
            ...
            OperatorTableBuilder.Sync(args =>
            {
                if (args[0] is not Value.Object o)
                {
        ...
        b.AddFunction(
            ...
            async (args, ctx) =>
            {
                if (args[0] is not Value.Object o || args[1] is not Value.Function fn)
                {
                    return Value.Undefined.Instance;
                }
        ...
        b.AddFunction(
            ...
            OperatorTableBuilder.Sync(args =>
            {
                ...
                IEnumerable<string> keys = ExtractKeys(args[1]);
                var result = new Dictionary<string, Value>(StringComparer.Ordinal);
                foreach (string k in keys)
                {
                    if (o.Props.TryGetValue(k, out Value? v))
                    {
                        result[k] = v;
                    }
                }
        ...
        b.AddFunction(
            ...
            OperatorTableBuilder.Sync(args =>
            {
                ...
                string sep = args.Length >= 2 && args[1] is Value.String s ? s.V : "_";
                var result = new Dictionary<string, Value>(StringComparer.Ordinal);
    ...
    private static IEnumerable<string> ExtractKeys(Value v) =>
        v switch
        {
    ...
    private static void FlattenObject(
        ...
        string prefix,
        string sep,
        Dictionary<string, Value> result
    )
    {
        foreach (KeyValuePair<string, Value> kv in o.Props)
        {
            string key = prefix.Length == 0 ? kv.Key : prefix + sep + kv.Key;
