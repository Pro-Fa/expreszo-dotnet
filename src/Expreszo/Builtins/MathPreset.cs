...
internal static class MathPreset
{
    ...
    private static void RegisterUnary(OperatorTableBuilder b)
    {
        ...
        b.AddUnary("length", OperatorTableBuilder.Sync(args =>
        {
            return args[0] switch
            {
                ...
                Value.Number n => Value.Number.Of(n.V.ToString("R", System.Globalization.CultureInfo.InvariantCulture).Length),
    ...
    {
        b.AddUnary(name, OperatorTableBuilder.Sync(args =>
        {
            if (args[0] is Value.Undefined) return Value.Undefined.Instance;
            var v = CorePreset.ToNumber(args[0]);
    ...
    {
        b.AddFunction("atan2", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 2 || args[0] is Value.Undefined || args[1] is Value.Undefined)
                return Value.Undefined.Instance;
            return Value.Number.Of(Math.Atan2(CorePreset.ToNumber(args[0]), CorePreset.ToNumber(args[1])));
        }));

        b.AddFunction("clamp", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 3) throw new ExpressionArgumentException("clamp requires 3 arguments");
            if (args[0] is Value.Undefined) return Value.Undefined.Instance;
            var v = CorePreset.ToNumber(args[0]);
            var lo = CorePreset.ToNumber(args[1]);
            var hi = CorePreset.ToNumber(args[2]);
            return Value.Number.Of(Math.Clamp(v, lo, hi));
        }));

        b.AddFunction("fac", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 1 || args[0] is Value.Undefined) return Value.Undefined.Instance;
            var v = CorePreset.ToNumber(args[0]);
            if (v < 0 || v != Math.Floor(v)) throw new EvaluationException("fac requires a non-negative integer");
            double r = 1;
            for (var i = 2; i <= v; i++) r *= i;
            return Value.Number.Of(r);
        }));

        b.AddFunction("gamma", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 1 || args[0] is Value.Undefined) return Value.Undefined.Instance;
        ...
        b.AddFunction("hypot", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length == 0) return Value.Undefined.Instance;
            double sum = 0;
            foreach (var a in args)
            {
                if (a is Value.Undefined) return Value.Undefined.Instance;
                var v = CorePreset.ToNumber(a);
                sum += v * v;
            }
        ...
        b.AddFunction("pow", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 2 || args[0] is Value.Undefined || args[1] is Value.Undefined) return Value.Undefined.Instance;
            return Value.Number.Of(Math.Pow(CorePreset.ToNumber(args[0]), CorePreset.ToNumber(args[1])));
        }));

        var rng = new Random();
        b.AddFunction("random", OperatorTableBuilder.Sync(args =>
        {
            var upper = args.Length == 0 || args[0] is Value.Undefined ? 1.0 : CorePreset.ToNumber(args[0]);
            return Value.Number.Of(rng.NextDouble() * upper);
        }));

        b.AddFunction("roundTo", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 1 || args[0] is Value.Undefined) return Value.Undefined.Instance;
            var v = CorePreset.ToNumber(args[0]);
            var digits = args.Length >= 2 && args[1] is not Value.Undefined ? (int)CorePreset.ToNumber(args[1]) : 0;
            return Value.Number.Of(Math.Round(v, digits, MidpointRounding.AwayFromZero));
        }));

        // ---------- statistics ----------

        b.AddFunction("sum", OperatorTableBuilder.Sync(args => AggregateNumbers(args, "sum",
            nums => { double s = 0; foreach (var n in nums) s += n; return s; })));

        b.AddFunction("mean", OperatorTableBuilder.Sync(args => AggregateNumbers(args, "mean",
            nums => { if (nums.Count == 0) return double.NaN; double s = 0; foreach (var n in nums) s += n; return s / nums.Count; })));

        b.AddFunction("median", OperatorTableBuilder.Sync(args => AggregateNumbers(args, "median",
            nums =>
            {
                if (nums.Count == 0) return double.NaN;
                var sorted = nums.OrderBy(n => n).ToArray();
                var mid = sorted.Length / 2;
                return (sorted.Length % 2 == 0) ? (sorted[mid - 1] + sorted[mid]) / 2 : sorted[mid];
            })));

        b.AddFunction("variance", OperatorTableBuilder.Sync(args => AggregateNumbers(args, "variance", Variance)));
        b.AddFunction("stddev", OperatorTableBuilder.Sync(args => AggregateNumbers(args, "stddev",
            nums => Math.Sqrt(Variance(nums)))));

        b.AddFunction("mostFrequent", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 1 || args[0] is not Value.Array arr) return Value.Undefined.Instance;
        ...
        b.AddFunction("percentile", OperatorTableBuilder.Sync(args =>
        {
            if (args.Length < 2 || args[0] is not Value.Array arr) return Value.Undefined.Instance;
            var p = CorePreset.ToNumber(args[1]) / 100.0;
            var nums = arr.Items.Select(ToNumberOrNaN).Where(d => !double.IsNaN(d)).OrderBy(d => d).ToArray();
            if (nums.Length == 0) return Value.Undefined.Instance;
            var rank = p * (nums.Length - 1);
            var lo = (int)Math.Floor(rank);
            var hi = (int)Math.Ceiling(rank);
            if (lo == hi) return Value.Number.Of(nums[lo]);
            var weight = rank - lo;
    ...
    private static Value MinMax(Value[] args, bool takeMax)
    {
        ...
        foreach (var n in numbers)
        {
            ...
            if (acc is null || (takeMax ? n > acc : n < acc)) acc = n;
        }
        return acc is null ? Value.Undefined.Instance : Value.Number.Of(acc.Value);
    }

    private static Value AggregateNumbers(Value[] args, string fnName, Func<List<double>, double> aggregate)
    {
        if (args.Length < 1 || args[0] is not Value.Array arr) return Value.Undefined.Instance;
        var nums = new List<double>(arr.Items.Length);
        foreach (var item in arr.Items)
        {
            var n = ToNumberOrNaN(item);
            if (!double.IsNaN(n)) nums.Add(n);
        }
        if (nums.Count == 0) return Value.Undefined.Instance;
        var result = aggregate(nums);
    ...
    private static double ToNumberOrNaN(Value v) => v switch
    {
        Value.Number n => n.V,
    ...
    private static double Gamma(double x)
    {
        ...
        var a = GammaCoeffs[0];
        var t = x + 7.5;
        for (var i = 1; i < GammaCoeffs.Length; i++)
        {
            a += GammaCoeffs[i] / (x + i);
        }
