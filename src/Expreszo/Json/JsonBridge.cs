...
public static class JsonBridge
{
    ...
    /// </list>
    /// </remarks>
    public static Value FromJson(JsonElement element) => element.ValueKind switch
    {
    ...
    public static void WriteValue(Utf8JsonWriter writer, Value value)
    {
        ...
        switch (value)
        {
            ...
            case Value.Object o:
                ...
                break;
            case Value.Function:
                throw new InvalidOperationException("Cannot serialise Value.Function to JSON: functions are not JSON-representable.");
            default:
                throw new InvalidOperationException($"Unknown Value variant: {value.GetType().Name}");
        }
    }
    ...
    /// </summary>
    public static void WriteObject(Utf8JsonWriter writer, IReadOnlyDictionary<string, Value> properties)
    {
        ArgumentNullException.ThrowIfNull(writer);
    ...
    {
        var builder = ImmutableArray.CreateBuilder<Value>(element.GetArrayLength());
